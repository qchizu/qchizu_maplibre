...
## 更新履歴
- 2024.5.26 試験公開中のCS立体図表示機能において、描画パラメータを更新して小縮尺での表示を改善しました。また、標高データがないエリアは青く表示するようにしました。
- 2024.5.19 サイドバーのデザインを変更しました。試験公開中のCS立体図表示機能において、「表現する地形のスケール」を変更できるようになりました（具体的には、曲率の平滑化に用いるガウシアン関数の標準偏差に乗じる係数をスライダーで調整できるようにしました）。
- 2024.5.12 標高タイルからCS立体図を表示する機能の試験公開を開始しました。
- 2024.4.21 JavaScriptのライブラリ”Svelte”を使ってリニューアルし、サイドバーを設けてより使いやすいUIとなりました。
...
### src/protocols/
- `dem2CsProtocol.js` : DEMデータからCS立体図を作成するプロトコル【試験公開中の機能】
...
### CS立体図（試験公開中）
MapLibre GL JSのaddProtocolを利用して、標高タイルから標高値を計算し、傾斜量、曲率を計算し画像を合成して表示しています。

なお、曲率はガウシアンフィルターにより平滑化した標高値を用いて計算しますが、平滑化の計算量が多いため、TensorFlow.jsを用いて動作の高速化を図っています。
...
今後は、さらなる表示の高速化、メモリやキャッシュの使用量の最適化などを行う予定です。
